...
import { RULES_DATA } from '@/lib/rules';
...
export default function VaultPage() {
  ...
  const [showCopyNote, setShowCopyNote] = useState(false); // New state for the corner note
  ...
  const handleProcess = (manualResult?: string) => {
    // If manualResult is passed, it means we are just re-copying existing text
    if (manualResult) {
    ...
    const cleanKey = ruleKey.toUpperCase().trim();
    const definition = RULES_DATA[cleanKey];
    ...
    // Show main overlay status
    setStatus(t.copy);
    // Show the green corner note
    setShowCopyNote(true);
...
{/* HEADER BAR - FORCED LTR FOR BUTTON POSITIONS */}
<div className="fixed top-8 left-0 right-0 px-8 flex flex-row justify-between items-center z-50" dir="ltr">
  ...
  {/* MIDDLE: Welcome Username - DYNAMIC DIRECTION */}
  <div className="flex flex-col items-center text-center" dir={t.dir}>
    <div className="flex items-center gap-2">
      <span className="text-[10px] text-blue-300/40 uppercase tracking-[0.3em] font-bold whitespace-nowrap">
        {t.welcome}
      </span>
      <span className="text-[11px] text-blue-400 font-black uppercase tracking-widest drop-shadow-[0_0_8px_rgba(96,165,250,0.5)] whitespace-nowrap">
        {username}
      </span>
    </div>
  ...
  {/* RIGHT: Always Language */}
  <button 
    onClick={toggleLang}
    className="glass-card w-[100px] py-2 text-[10px] font-bold border border-white/10 hover:bg-white/10 transition-all flex items-center justify-center uppercase tracking-widest"
  >
    {t.lang}
  </button>
</div>
      ...
      <div className="glass-card p-10 max-w-2xl w-full space-y-8 mt-12 border border-white/5">
          ...
          {['normal', 'LocalOOC', 'dm'].map((m) => (
            <button
              ...
              className={`px-5 py-2 rounded-xl text-[10px] font-bold uppercase transition-all border ${
                mode === m ? 'bg-blue-600 border-blue-400 text-white' : 'bg-white/5 border-white/10 text-blue-300/50'
              }`}
            >
              {m === 'normal' ? t.normal : m}
            </button>
        ...
        <div className="space-y-6">
          ...
          <div className="min-h-[140px] flex flex-col items-center justify-center p-8 bg-black/40 rounded-3xl border border-white/5 relative group">
            ...
            {/* Corner Feedback - THE ONLY SUCCESS INDICATOR */}
            {showCopyNote && (
              <div className="absolute top-4 right-6 flex items-center gap-2 text-emerald-400 animate-in fade-in slide-in-from-right-2 duration-300">
                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 -translate-y-[1px] animate-pulse"></div>
                <span className="text-[10px] font-bold uppercase tracking-widest leading-none">
                  {t.copy || "COPIED SUCCESSFULLY"}
                </span>
              </div>
            )}
            ...
            {displayResult && (
              <button 
                onClick={() => handleProcess(displayResult)}
                className="mt-4 px-4 py-1.5 bg-white/5 border border-white/10 rounded-full text-[9px] font-bold uppercase tracking-widest text-white/40 hover:text-white hover:bg-white/10 hover:border-white-20 transition-all flex items-center gap-2"
              >
            ...
            {/* ERROR OVERLAY ONLY (Red) */}
            {status && isError && (
              <div className="absolute inset-0 flex items-center justify-center backdrop-blur-sm rounded-3xl animate-in fade-in zoom-in duration-200 bg-red-900/40">
